...
async function removeAllLiquidity() {
	amount = await exchange_contract
		.connect(provider.getSigner(defaultAccount))
		.removeAllLiquidity();
...
async function getExchangeRates(maxSlippagePct) {
	reserves = await exchange_contract
		.connect(provider.getSigner(defaultAccount))
		.getLiquidity();
	current_exchange_rate = Math.round(
		(exchange_rate_multiplier * reserves[0]) /
			Number(ethers.utils.formatEther(reserves[1]))
	);
	change_exchange_rate = Math.round(
		(exchange_rate_multiplier * reserves[0] * Number(maxSlippagePct)) /
			(Number(ethers.utils.formatEther(reserves[1])) * 100)
	);
...
async function swapTokensForETH(amountToken, maxSlippagePct) {
	reserves = await exchange_contract
		.connect(provider.getSigner(defaultAccount))
		.getLiquidity();
	max_exchange_rate = Math.round(
		(Number(maxSlippagePct) *
			exchange_rate_multiplier *
			(reserves[0] + Number(amountToken))) /
			(Number(ethers.utils.formatEther(reserves[1])) * 100) +
			(exchange_rate_multiplier * (reserves[0] + Number(amountToken))) /
				Number(ethers.utils.formatEther(reserves[1]))
	);

	balance = await token_contract
		.connect(provider.getSigner(defaultAccount))
		.balanceOf(defaultAccount);
	await token_contract
		.connect(provider.getSigner(defaultAccount))
		.approve(exchange_address, balance);
	await exchange_contract
		.connect(provider.getSigner(defaultAccount))
		.swapTokensForETH(amountToken, max_exchange_rate);
...
async function swapETHForTokens(amountEth, maxSlippagePct) {
	...
	max_exchange_rate = Math.round(
		(Number(maxSlippagePct) *
			(exchange_rate_multiplier *
				(Number(ethers.utils.formatEther(reserves[1])) + Number(amountEth)))) /
			(reserves[0] * 100) +
			(exchange_rate_multiplier *
				(Number(ethers.utils.formatEther(reserves[1])) + Number(amountEth))) /
				reserves[0]
	...
	await exchange_contract
		.connect(provider.getSigner(defaultAccount))
		.swapETHForTokens(max_exchange_rate, {
